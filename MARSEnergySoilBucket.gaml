    ...
    reflex saving_output when: log_output and year >= log_first_year and (
    	...
    	loop n over: cell   {
    		int sol <-  (cycle -1) / (2 * step_sol);
    			...
    			//if (n.id_cell = 1499 or n.id_cell = 1513 or n.id_cell = 333 or n.id_cell = 3090){
...
//										  Ks-1 
		...
		/// ground_delta_En <- k_reg   * (prevTs - subground_Ts) / deepSoil
		/// subground_Ts <- subground_Ts + delta_t * k_reg   * (prevTs - subground_Ts) /  (cSoil * soilDensity )  
...
//		monitor "HS"     name: hs value: hadleyS;
		...
		monitor "Cell 1789 (SPole) frozen" name: SPfrozen value: cell[1789].frozenCO2;
		monitor "Cell 2860 (NPole) frozen" name: NPfrozen value: cell[2860].frozenCO2;
